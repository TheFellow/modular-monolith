...
{
    [Fact]
    public void DefaultDatabaseLivesOutsideTheApplicationBundle()
    {
        RecordingRuntime runtime = new();

        int exit = DesktopCommandLine.Build(runtime).Parse([]).Invoke();

        Assert.Equal(ErrorCatalog.ExitSuccess, exit);
        Assert.Equal(Path.GetFullPath(DesktopOptions.DefaultDatabasePath), runtime.Options?.DatabasePath);
        Assert.DoesNotContain(".app/", runtime.Options?.DatabasePath, StringComparison.OrdinalIgnoreCase);
    }
